...
export class PriceDataCollector {
  ...
  private logRequestDetails(website: string, attempt: number) {
    console.log(`[Request Details] Attempt ${attempt}:
`, {
      website,
  ...
  private async makeRequest(website: string, attempt: number = 1): Promise<any> {
    ...
    try {
      ...
      console.log(`[Response Success] Website: ${website}
`, {
      ...
      const duration = Date.now() - startTime;
      console.error(`[API Error] Website: ${website}, Attempt: ${attempt}
`, {
        duration: `${duration}ms`,
